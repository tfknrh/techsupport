...
class ExifMetadata {
  ...
  final jpegInterchangeFormatLength;
  ...
  final isoSpeedLatitudezzz;

  /// Shutter speed. The unit is the APEX setting.
  final shutterSpeedValue;
  ...
  final focalPlaneResolutionUnit;
  ...
  /// reader.
  final deviceSettingDescription;

  /// This tag indicates the distance to the subject.
  final subjectDistanceRange;
...
class GpsMetadata {
  ...
  final gpsDifferential;

  /// This tag indicates horizontal positioning errors in meters.
  final gpsHPositioningError;

  /// Indicates the identification of the Interoperability rule.
  final interoperabilityIndex;
...
_castAsDouble(dynamic variable) {
  if (variable == null) {
    return null;
  }
...
_castAsString(dynamic variable) {
  if (variable == null) {
    return null;
  }
...
List _castAsIntMap(List<dynamic> list) {
  if (list == null) {
    return null;
  }

  return list.map((n) => _castAsInt(n)).toList();
}

List _castAsDoubleMap(List<dynamic> list) {
  if (list == null) {
    return null;
  }

  return list.map((n) => _castAsDouble(n)).toList();
}
